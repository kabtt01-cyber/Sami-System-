...
-- Create Isolation Policies (Enable simple public or request-isolation policies)
-- Note: To keep the setup safe and fully functional for both web client and server access, 
-- we allow standard operations while isolating queries where required.
-- If no custom JWT claims are set up, we default policies to ALLOW ALL, which is safe for sandboxed prototypes
-- while providing the exact blueprint for Row-Level security.
